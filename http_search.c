		...
		for (i=0; i < nodeset->nodeNr; i++) {
            ...
            if ( href != NULL && !strncmp((const char *)href, "http", 4) ) {
				...
				strcpy(href_copy, (char*)href);
   				...
   				pthread_mutex_lock(p_frontier_mutex);
					pushed = push(p_frontier_vec, href_copy);
					if (pushed) {
						pthread_cond_signal(p_wake_condition);
					} else {
						free(href_copy);
					}
				pthread_mutex_unlock(p_frontier_mutex);
            }
            xmlFree(href);
        }
        xmlXPathFreeObject (result);
...
int process_html(CURL *curl_handle, RECV_BUF *p_recv_buf,
    ...
    find_http(p_recv_buf->buf, p_recv_buf->size, follow_relative_link, url,
			  p_frontier_vec, p_frontier_mutex, p_wake_condition, n);
...
int process_png(CURL *curl_handle, RECV_BUF *p_recv_buf,
    ...
    if ( eurl != NULL) {
		...
		if (is_PNG) {
			if (atomic_fetch_add(p_png_counter, 1) >= *p_max_png_count) {
			...
			pthread_mutex_lock(p_png_mutex);
				pushed = push(p_png_vec, eurl_copy);
			pthread_mutex_unlock(p_png_mutex);
			if (!pushed){
				free(eurl_copy);
			}
		}
...
int process_data(CURL *curl_handle, RECV_BUF *p_recv_buf,
    ...
    if ( strstr(ct, CT_HTML) ) {
        ...
        return process_png(curl_handle, p_recv_buf, p_png_vec, p_png_mutex, n,
						   p_png_counter, p_max_png_count);
    } else {
...
//	pthread_mutex_lock(p_png_mutex);
    ...
    process_data(curl_handle, &recv_buf, p_png_vec, p_frontier_vec,
				 p_png_mutex, p_frontier_mutex, p_wake_condition, n,
				 p_png_counter, p_max_png_count);

    /* cleaning up */
    cleanup(curl_handle, &recv_buf);
//	printf("Free p_URL in http\n");
//	free(p_URL);
	//xmlCleanupParser();
//	pthread_mutex_unlock(p_png_mutex);
